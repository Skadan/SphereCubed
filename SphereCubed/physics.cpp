...
//! \param timeInterval is the time interval in mS between Physics updates.
...
//! \param position is the current internal position of the player for time imterval.
//! \param velocity is the current internal velocity of the player for time imterval.
//! \return void
void Physics::collisionDetection(QVector3D &position, QVector3D &velocity )
{
    ...
    {
        //! - Test for collisons along the path to travel.
...
void Physics::collisionDetectionCornerTest( QVector3D &position, QVector3D &velocity )
{
    ...
    //! Determine where destination would be without any collisions occuring.
...
void Physics::collisionResponse(QVector3D & position, QVector3D &velocity )
{
    ...
    {
        //! Then no collision occured and the Player position and rotation can move forward the entire distance,
        position += velocity;
        roll( velocity );
        velocity = QVector3D(0.0f,0.0f,0.0f);
    } // if( mCollisionData.distance == FLT_MAX )
    //! Else a collision occured.
    else
    {
...
float Physics::distanceOnRayToPlane( QVector3D & rayStart, QVector3D & rayDirection, QVector3D & planePoint, QVector3D & planeNormal )
{
    ...
    //! If the ray is parrallel to the plane.
    ...
    //! If collision occured behind ray start.
    if( distance < -FLT_EPSILON )
    {
        //! - Return maximum float value, no intersection will occur.
        return FLT_MAX;
...
void Physics::forces()
{
    ...
    //! Apply the sum of accelerations to the linear velocity for the time step.
    mPlayer.velocity() += acceleration * mTimeInterval;

    //! If the Player linear velocity is now greater then the defined terminal linear velocity.
    if( mPlayer.velocity().length() > mPlayer.terminalLinearVelocity() )
    {
        //! Cap the linear velocity to the terminal linear velocity.
        mPlayer.velocity() = mPlayer.velocity().normalized() * mPlayer.terminalLinearVelocity();
    } // if( mPlayer.linearVelocity().length() > mPlayer.terminalLinearVelocity() )

    //! If the Player linear velocity in the X direction is less then the minimum defined threshold.
    if( qAbs( mPlayer.velocity().x() ) < MINIMUM_LINEAR_VELOCITY )
    {
    ...
    } // if( qAbs( mPlayer.linearVelocity().x() ) < MINIMUM_LINEAR_VELOCITY )

    //! If the Player linear velocity in the Y direction is less then the minimum defined threshold.
    if( qAbs( mPlayer.velocity().y() ) < MINIMUM_LINEAR_VELOCITY )
    {
        ...
        //! Snap to Cube sufrace to prevent gravity from being applied and causing oscillations caused by limited precision.
        mPlayer.position().setY( qRound( mPlayer.position().y() ) );
    } // if( qAbs( mPlayer.linearVelocity().y() ) < MINIMUM_LINEAR_VELOCITY )

    //! If the Player linear velocity in the Z direction is less then the minimum defined threshold.
    if( qAbs( mPlayer.velocity().z() ) < MINIMUM_LINEAR_VELOCITY )
    {
    ...
    } // if( qAbs( mPlayer.linearVelocity().z() ) < MINIMUM_LINEAR_VELOCITY )
} // Physics::forces()

//! Move the player for position in time.
//! \return void
//! \sa Physics, Player, Level
void Physics::movement()
{
    ...
    //! Move the plaer over the distance.
    while( intervalDistance.length() > 0 )
    {
        //! - Check for collisions with the Level of the distsnce to travel for the time interval.
        collisionDetection(intervalPosition, intervalDistance);
        //! - Move the Player to first point of collision or to end of distance travel for time interval.
